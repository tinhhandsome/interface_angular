...
export class PlPoComponent extends DefaultComponentBase implements OnInit, AfterViewInit {
    ...
    branchTypes: any[] = [
        ...
        {
            value: 'HS',
            display: this.l('Headquarters')
        },
        {
            value: 'CNPGD',
            display: this.l('CNPGD')
        }
    ]
    ...
    previewTemplate() {
        ...
        let values = this.GetParamsFromFilter({
            ...
            fullName: this.appSession.user.name,
            duration: ((this.filterInput.FROM_DATE? this.l('FromDate') + ' ' + this.filterInput.FROM_DATE.format('DD/MM/YYYY') : '') + ' ' + this.l('ToDate').toLocaleLowerCase() + ' ' + this.filterInput.TO_DATE.format('DD/MM/YYYY')).toUpperCase(),
