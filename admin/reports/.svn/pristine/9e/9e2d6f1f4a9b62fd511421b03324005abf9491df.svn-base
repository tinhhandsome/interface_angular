...
export class AssListAssetLiquidComponent extends DefaultComponentBase implements OnInit, AfterViewInit {
    ...
    getReportInfo(): ReportInfo {
        ...
        var type = this.filterInput.TYPE_ID ? this.filterInput.TYPE_ID.toString() : this.l('Asset').toLowerCase();

        reportInfo.values = this.GetParamsFromFilter({
            A4: (this.l('List') + ' ' + type + ' ' + this.l('LiquidRequest').toLowerCase() + ' ' + this.l('To') + ' ' + dateFormatter.transform(curDate)).toUpperCase(),
