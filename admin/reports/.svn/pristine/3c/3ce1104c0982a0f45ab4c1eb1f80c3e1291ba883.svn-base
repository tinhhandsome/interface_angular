...
export class CCLDBc08_6Component extends DefaultComponentBase implements OnInit, AfterViewInit {
    ...
    exportToExcel() {
        ...
        if (this.filterInput.FROMDATE) {
            dateMessage = 'Từ ngày ' + this.filterInput.FROMDATE.format('DD/MM/YYYY') + ' đến ngày ' + this.filterInput.TODATE.format('DD/MM/YYYY');

        } else {
            dateMessage = 'Đến ngày ' + this.filterInput.TODATE.format('DD/MM/YYYY');
