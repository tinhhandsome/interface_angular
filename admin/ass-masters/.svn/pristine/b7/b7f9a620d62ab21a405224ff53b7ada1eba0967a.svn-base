...
@Component({
    selector: 'ass-add-new-export-use',
    templateUrl: './ass-add-new-export-use.component.html',
    encapsulation: ViewEncapsulation.None,
    changeDetection: ChangeDetectionStrategy.OnPush,
    animations: [appModuleAnimation()]
})
export class AssAddNewExportUseComponent extends ChangeDetectionComponent implements AfterViewInit {
    constructor(
        injector: Injector,
    ) {
        super(injector);
    ...
    @ViewChild('exportUseForm') exportUseForm: NgForm;
    ...
    // Ngày bắt đầu khấu hao không được nhỏ hơn ngày nhập
    amorT_START_DATE_checkValid() {
        if (this.inputModel.amorT_START_DATE.diff(this.inputModel.buY_DATE) < 0) {
            this.exportUseForm.controls['amorT_START_DATE']['errorMessage'] = this.l('AmortStartDate') + ' ' + this.l("NotLessThan") + ' ' + this.l("InputDate").toLocaleLowerCase();
        }
        else {
            this.exportUseForm.controls['amorT_START_DATE']['errorMessage'] = undefined;
        }
        this.updateView();
    }
    ...
    checkValid() {
        let isValid = true;
        if (this.exportUseForm.invalid) {
            isValid = false;
        }
        this.amorT_START_DATE_checkValid();
        if (this.exportUseForm.controls['amorT_START_DATE']['errorMessage']) {
            isValid = false;
        }
    ...
    exportUseChange() {
        if (!this.isExportUse) {
            ...
            this.editForm.controls['entrY_BOOKED'].setValue('Y');
        }
